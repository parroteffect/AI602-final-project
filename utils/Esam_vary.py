...
class ESAM_vary(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=0.05,beta=1.0,gamma=1.0,adaptive=False,**kwargs):
        ...
        super(ESAM_vary, self).__init__(params, defaults)
    ...
    def step(self):
        ...
        with torch.no_grad():
            ...
            instance_sharpness_var = torch.var(instance_sharpness)
            #codes for sorting 
            prob = self.gamma
            if(instance_sharpness_var < 0.008):
                prob = self.gamma-0.1
            if prob >=0.99:
        ...
        self.returnthings = (predictions,return_loss,instance_sharpness.mean())
